...
def map():
	...
	record = data[0]
	print("\nNumber of Fields in one Record is:", len(record), "\n")
	...
	for i in record:
		...
		while temp<1 or temp>19:
			...
			if temp<1 or temp>19:
				print("\n\nInvalid input. Input must be between 1 and 20 (inclusive).\n\n")
				temp=-1
...
def main():
	...
	print ("----------\nWelcome to RadioTools.py:\n\nCaveat Emptor:\nPlease double check all documents created by this script for accuracy in importing.")
	print ("Some fields, such as dates are not formatted by this program and thus will appear exactly as they were in the original document.\n----------\n")
	if len(sys.argv) > 1 :
		if sys.argv[1] == "-h":
			help()
		elif sys.argv[1] == "-i":
